...
fetch("https://green-pc.vercel.app/users" , {
  method:"POST" ,
  headers:{
...
.then(data => {
 ...
 toast.info("Got to dashboard and click on Get Start Button !! ") ;
 navigate("/") ;
 }
}).catch(error => {
  setLoading(false) ;
...
method:"POST" , 
 body:formData , 
  ...
  .then(data => {
   ...
   createUser(userFormData?.email , userFormData?.password,userFormData?.name,profileImage)
   .then((result) => {
...
<input id="inputColor" type="text" placeholder="Please enter your  full name" 
{...register("name" , {
required : true , 
})}
...
<input id="inputColor" type="text" placeholder="Please enter your  email" {...register("email" , {
required:true , 
})} className="input input-bordered" />
...
<input id="inputColor" type="password" placeholder="Please enter your password" 
{...register("password" , {
...
<select id="inputColor" className="select select-bordered" 
{...register("role" , {
  required:true , 
})}>
